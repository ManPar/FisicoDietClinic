...
public class UploadActivity extends AppCompatActivity {
        ...
        after.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (EasyPermissions.hasPermissions(getApplicationContext(), galleryPermissions)) {
                    ...
                    EasyPermissions.requestPermissions(getApplicationContext(), "Access for storage",
                            101, galleryPermissions);
                }
                ...
                try {
                    final Runnable runnable = new Runnable() {
                        ...
                        public void run() {
                            try {
                                if (IMAGE_COUNT == 2) {
                                    InputStream in = getApplicationContext().getContentResolver().openInputStream(selectedImage);
                                    Map uploadResult = cloudinary.uploader().upload(in, ObjectUtils.emptyMap());
                                    InputStream in1 = getApplicationContext().getContentResolver().openInputStream(selectedImage1);
    ...
    public void showToast() {
        ...
        if (CASE == 1) {
            ...
            Toast.makeText(getApplicationContext(), "Sorry!Images could not be Uploaded", Toast.LENGTH_LONG).show();
        } else if (CASE == 3) {
            Toast.makeText(getApplicationContext(), "Please attach both the images", Toast.LENGTH_LONG).show();
        ...
        try {
            ...
            if (BEFORE_TAG == 1) {
                ...
                if (requestCode == RESULT_LOAD && resultCode == RESULT_OK
                    ...
                    // Get the cursor
                    Cursor cursor = getApplicationContext().getContentResolver().query(selectedImage1,
                            filePathColumn, null, null, null);
                    // Move to first row
                    cursor.moveToFirst();
